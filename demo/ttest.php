...
<div class="card card-info k-rtl" >
              ...
              <div class="card-header">
                ...
                <h3 style="float:right;" class="card-title"> إنشاء شاشات النظام</h3>
              </div>
              <div class="card-body">
	...
	<form  method="POST">
      	<div class="row">
                      ...
                      <!-- select -->
                        ...
                        <label  class="col-form-label" for="screen_id"><i ></i> معرف الشاشة</label>
                    <input style="margin:5px;"  type="text" class="col-sm-6  form-control is-valid"  name="screen_id"   id="screen_id" placeholder="الاسم" disabled>  
                      ...
                      <!-- select -->
                        ...
                        <label  class="col-form-label" for="screen_name"><i ></i> اسم الشاشة :</label>
                    <input style="margin:5px;"  type="text" class="col-sm-12  form-control is-valid"  name="screen_name"   id="screen_name" placeholder="  اسم الشاشة" enabled>  
                      </div>
                      </div>
                      ...
                      <div class="row"> 
<button style="margin:5px 5px 5px 5px; "   type="button" class="btn btn-primary has-spinner  all1"  name="newwasc"><span text-wating="انتظر"icon-button="k-icon k-i-plus-outline"  class="k-icon k-i-plus-outline"></span> جديد</a>
<button    style="margin :5px 5px 5px 5px; "     type="button" class="btn btn-success has-spinner  all2" id="savesc" name="savesc" data-dismiss="modal"><span text-wating="انتظر.." icon-button="k-icon k-i-save" class="k-icon k-i-save"></span> حفظ</button>
  <button    style="margin :5px 5px 5px 5px; "     type="button" class="btn  btn-warning has-spinner  all3" id="savesjc" name="savesjc" data-dismiss="modal"   ><span  text-wating="انتظر.." icon-button="k-icon k-i-pencil"  class="k-icon k-i-pencil"></span> تعديل</button>
...
schema: {
                        model: {
                            id: "screen_id",
                            fields: {
                                screen_id: { editable: false, nullable: true },
...
aformButton1 = document.querySelector(".all1");
aformButton2 = document.querySelector(".all2");
aformButton3 = document.querySelector(".all3");
...
let screen_idd= $("input[name*='screen_id']");//$("firstname']");
screen_idd.val(dataItem.screen_id);


//idd.val(e.target.dataset.user_id);
let screen_namee = $("input[name*='screen_name']");//$("firstname']");
screen_namee.val(dataItem.screen_name);
...
function inserta(typcurdd,iddelete,rowbtn){
var screen_ida= $('#screen_id').val();
if(typcurdd=="1"){
	$(".all2").buttonLoaderr('start');
	screen_ida="0";
}
if(typcurdd=="2"){
	$(".all3").buttonLoaderr('start');
	//screen_ida="0";
}
if(typcurdd=="3"){
	screen_ida=iddelete;
...
screen_enba = "1";
   ...
   $.ajax({
        url: 'http://'+ippx+':8080/api/registerscreen.php', // <-- point to server-side PHP script 
        dataType: 'text',  // <-- what to expect back from the PHP script, if anything
        cache: false,
...
//xhr.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=UTF-8");
    xhr.setRequestHeader ("Authorization", "Bearer " + rrr);
xhr.setRequestHeader("screen_id",btoa(unescape(encodeURIComponent(screen_ida))));
xhr.setRequestHeader("screen_name",btoa(unescape(encodeURIComponent(screen_namea))));
xhr.setRequestHeader("screen_enb",btoa(unescape(encodeURIComponent(screen_enba))));
...
//alert("اكسستوكن");
inserta(typcurdd,iddelete,rowbtn);
} else {
//eraseCookie("tokken");
if(ffdd.success=='1'){
if(typcurdd=="2"){
	$(".all3").buttonLoaderr('stop');
}
if(typcurdd=="1"){
	$(".all2").buttonLoaderr('stop');
}
if(typcurdd=="3"){
...
inserta(3,dataItem.screen_id,e);
